...
module.exports = {
   createProduct:(req,res)=>{
        ...
        let { cat_id,name,desc,price,discount,prod_details,extra_details,createdBy,size }=req.body;
       connection.query(`INSERT INTO prod_details(prod_id,cat_id,prod_name,prod_desc,prod_price,prod_discount,prod_img,prod_details,extra_details,isActive,createdBy,size ) VALUES(0,${cat_id},'${name}','${desc}',${price},${discount},'${profileimg}',${JSON.stringify(prod_details)},${JSON.stringify(extra_details)},1,${createdBy},'${size}')`, (err, result) => {
            if (err) {
                res.status(500).send({ error: true, message: err });
            } else {
                req.files.map((img) => {
                    connection.query(`insert into images(img_id,img_url,prod_id) values (0,'${img.filename}',${result.insertId})`);
                })
